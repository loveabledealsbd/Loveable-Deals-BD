...
import { fetchOrdersByStoreId, updateOrderStatus as updateOrderStatusFirestore } from "@/lib/firestore/orders"
...
export default function StoreOrders() {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            if (store) {
                const data = await fetchOrdersByStoreId(store.id)
                setOrders(data)
            }
        } catch (error) {
                ...
                <div onClick={closeModal} className="fixed inset-0 flex items-center justify-center bg-black/50 text-slate-700 text-sm backdrop-blur-xs z-50" >
                    <div onClick={e => e.stopPropagation()} className="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6 relative">
                        ...
                        <div className="mb-4">
                            ...
                            <p><span className="text-green-700">Order Date:</span> {selectedOrder.createdAt ? new Date(selectedOrder.createdAt.seconds ? selectedOrder.createdAt.seconds * 1000 : selectedOrder.createdAt).toLocaleString() : ''}</p>
                        </div>
