...
import { fetchProductsByStoreId, toggleProductStock } from "@/lib/firestore/products"
...
export default function StoreManageProducts() {
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            if (store) {
                const data = await fetchProductsByStoreId(store.id)
                setProducts(data)
            }
        } catch (error) {
    ...
    const toggleStock = async (productId) => {
        try {
            ...
            await toggleProductStock(productId, product.inStock)
            // Update local state
                            ...
                            <tr key={product.id} className="border-t border-gray-200 hover:bg-gray-50">
                                ...
                                <td className="px-4 py-3 text-center">
                                    <div className="flex items-center justify-center gap-4">
                                        ...
                                        <Link href={`/store/update-product/${product.id}`} className="text-blue-500 hover:text-blue-700 transition" title="Edit Product">
